...
**Objective:** Download the complete CRA T4002 Self-employed Business guide from
canada.ca for RAG ingestion.

**Target URL:**
https://www.canada.ca/en/revenue-agency/services/forms-publications/publications/t4002/

**Output Directory:**
`/Users/manonjacquin/Documents_local/POCs/quickExpense-rag/cra_documents/cra_t4002e_rev24_dump`
...
1. Set up Python environment with `requests` and `beautifulsoup4` libraries
1. Configure HTTP headers with User-Agent to mimic standard browser
1. Download `t4002-1.html` (Table of Contents page) from base URL
1. Save raw HTML to output directory
...
1. Locate table of contents navigation element
1. Extract all `<a>` tags with href matching pattern `t4002-*.html`
1. Build list of unique absolute URLs (13 pages expected)
1. Print discovered URLs to console for manual verification
...
- All URLs match the base pattern:
  `https://www.canada.ca/en/revenue-agency/services/forms-publications/publications/t4002/t4002-*.html`
...
1. Use **clink gemini** (Gemini 2.5 Pro) to analyze page structure due to large size
...
1. Verify: Are there any sub-pages beyond the main 13 pages?
1. Create structure documentation for reference
...
1. Extract all CSS file URLs from `<link rel="stylesheet">` tags
1. Log CSS dependencies
1. Verify download immediately before proceeding
...
1. Save to output directory
1. **Use clink gemini** to analyze due to large page size
1. Extract CSS dependencies and internal links via gemini
...
1. Save to output directory
1. **Use clink gemini** to analyze due to large page size
1. Extract CSS dependencies and internal links
1. Verify content structure and nested sections
...
1. Save to output directory
1. Extract CSS dependencies
1. Log internal links
1. Verify download
...
1. **Use clink gemini** to analyze due to large page size
1. Extract CSS dependencies and internal links
1. Verify expense-related content
...
1. **Use clink gemini** to analyze due to large page size
1. Extract CSS dependencies and internal links
1. Verify capital cost allowance content
...
1. Save to output directory
1. Extract CSS dependencies
1. Verify capital gains content
...
1. Save to output directory
1. Extract CSS dependencies
1. Verify CCA rates table content
...
1. Save to output directory
1. Extract CSS dependencies
1. Verify inventory adjustment content
...
1. Save to output directory
1. Extract CSS dependencies
1. Verify GST/HST content for farmers and fishers
...
1. Save to output directory
1. Extract CSS dependencies
1. Verify digital services content
...
1. Save to output directory
1. Extract CSS dependencies
1. Verify additional information and resources content
...
1. Create `assets/` subdirectory in output directory
1. Download each unique CSS file
1. Save CSS files with original filenames to `assets/` directory
1. Handle both absolute and relative CSS URLs
...
1. For each of the 13 HTML files:
   ...
   - Example: `https://www.canada.ca/.../t4002-5.html#expenses` becomes
     `t4002-5.html#expenses`
   - Preserve anchor fragments (hash links)
   - Leave external links unchanged (links to other CRA guides, forms)
1. Save modified HTML, overwriting original
...
1. For each of the 13 HTML files:
   ...
   - Example: `/etc/designs/canada/wet-boew/css/theme.min.css` becomes
     `assets/theme.min.css`
   - Example: `https://www.canada.ca/etc/designs/.../style.css` becomes
     `assets/style.css`
1. Save modified HTML, overwriting previous version
...
- **Use Gemini for large pages:** Tasks 2.2, 2.3, 2.5, 2.6 involve large pages that may
  exceed token limits
...
- **Missing page numbers:** Note that t4002-7.html and t4002-13.html do not exist in the
  guide
- **No JavaScript required:** Static HTML content only needs CSS for formatting
- **Preserve anchor links:** Many internal links use fragments (e.g., `#section`) for
  navigation
