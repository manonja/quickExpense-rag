...
**QuickExpense RAG** is a Python library providing semantic search over Canadian Revenue Agency (CRA) business expense rules. The library combines keyword search (SQLite FTS5) and semantic search (sqlite-vec with BGE embeddings) using Reciprocal Rank Fusion (RRF) for hybrid search.

**Critical**: This project is NOT tax advice software. All user-facing APIs and outputs MUST include prominent legal disclaimers stating the content is informational only and users must consult qualified tax professionals.
...
4. **RRF fusion** - Reciprocal Rank Fusion merges rankings: `score = 1/(k + rank)`
...
- **Integrity**: SHA256 checksums verified on download, version compatibility checked on init

## Database Schema Contract

The schema (defined in `src/quickexpense_rag/data/schema.py`) defines the database structure:

**Tables**:
- `metadata` - Schema version, data version, build info
- `rules` - Main content table with citation_id (unique), content, metadata (province, business_type, expense_type)
...
**Key Constraint**: `citation_id` must be unique and follow pattern `S\d+-F\d+-C\d+-p\d+\.?\d*`
...
- **Meaningful messages**: Use conventional commits format (feat:, fix:, docs:, refactor:, test:)
...
### Immutability
All Pydantic models use `frozen=True` and `extra='forbid'` to ensure API contracts are enforced.
...
The BGE-small-en-v1.5 model produces 384-dimensional embeddings. Changing models requires full database rebuild. The model name is stored in database metadata for version tracking.
...
- **Schema version**: Major.Minor (e.g., "1.0") - breaking schema changes increment major
...
1. **Don't bundle database in package** - It's downloaded separately from GitHub Releases
2. **Don't skip disclaimers** - Every user-facing output needs legal warnings
3. **Don't break the schema contract** - Tests and runtime depend on consistent schema
4. **Use fixture DB for tests** - Don't require production data for development
5. **Embedding model changes require full rebuild** - Vector dimensions are fixed at 384
6. **Don't over-engineer** - Follow 80/20 principle, build what's needed now
