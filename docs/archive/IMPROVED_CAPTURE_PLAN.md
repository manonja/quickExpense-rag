...
1. **RULE definitions**: `<h3 id="tocch2ln9600">Line 9600 – Other income</h3>` (has
   anchor ID)
1. **RULE references**: `<span>line 9600</span>` (mentions line number, no anchor)
1. **TABLES**: `<table>` elements with structured data
1. **EXAMPLES**: `<section class="panel"><h4>Example</h4>` case studies
1. **GUIDANCE**: Paragraphs explaining concepts, definitions, procedures

### The Real Problem

Our current parser can't distinguish **rule definitions** from **rule references**,
causing duplicates. But fixing duplicates isn't the goal - it's a symptom of needing
...
**Test**: Run on t4002-4.html and t4002-6.html **Expected**: 16 rules from t4002-4, 2
from t4002-6 (no duplicates)

**Acceptance Criteria** (from Zen):

- **Primary**: Parser extracts exactly 16 rules from t4002-4.html and exactly 2 from
  t4002-6.html
- **Validation**: `uv run extract-rules t4002-4.html | grep -c "^- rule_number:"`
  returns 16; same for t4002-6 returns 2
...
- **Regression Test**: Must also test t4002-5.html still extracts 63 rules (no
  regression)

**Commit**: ✅ DONE - `b07fd2e`: "fix: distinguish rule definitions from references in
classic parser"
...
- **Primary**: `docs/CONTENT_PATTERNS.md` exists with two sections: "Rule Definitions"
  and "Rule References"
- **Content**: Each section has ≥1 complete HTML snippet (full `<h3>...</h3>` tag, not
  just attributes) + brief explanation
- **Contrast**: Include one negative example in "Rule Definition" section showing a
  reference and why it's NOT a definition
- **Validation**: Manual peer review - team member can distinguish definition from
...
**Actual Results**: Comprehensive documentation created with HTML examples, pattern
recognition table, and validation commands
...
- **Validation**:
  `cat output/**/rules.yml | grep "rule_number:" | sort | uniq -d | wc -l` returns 0
- **Failure**: Command outputs number >0 (any duplicates found)
- **Scope**: Automated validation only (no manual spot-check required)
- **Quality**: Structure validation only (no duplicates); content readability out of
  scope
- **Regression**: Must re-process t4002-5.html and verify 63 rules extracted (no
  regression)

**Commit**: ✅ DONE - `06aa44c`: "docs: add Phase 1 validation results with improved
extraction findings"
...
- t4002-5.html: 65 rules (improved from 63) - found 2 missed combined headings
  (LINE-8960, LINE-9899) ✅
...
1. **Structural Correctness**: `src/qe_tax_rag/extraction/ca/models.py` contains
   `ContentType` enum with `RULE` and `PRINCIPLE` members, and `ExtractedContent`
   Pydantic model as defined
1. **Successful Instantiation**: Unit test demonstrates `ExtractedContent` can be
   created with `content_type=ContentType.PRINCIPLE`
   - **Validation**:
     `pytest tests/unit/extraction/ca/test_models.py::test_create_principle_content`
1. **Type Validation**: Unit test confirms invalid `content_type` string (e.g.,
   `"COMMENT"`) raises `pydantic.ValidationError`
   - **Validation**:
     `pytest tests/unit/extraction/ca/test_models.py::test_invalid_content_type`
...
1. **Function Existence**: Functions `extract_line_references` and `extract_principles`
   exist in `principle_parser.py`
1. **Non-Empty Extraction**: `extract_principles` on t4002-6.html returns non-empty list
   - **Validation**: Test asserts `len(principles) > 0`
1. **Correct Content Typing**: Every returned object has
   `content_type == ContentType.PRINCIPLE`
   - **Validation**: Test iterates and asserts all items match
1. **Reference-Text Integrity**: For known principle (e.g., "Enter on line 9925..."),
   extracted object contains:
   - `text` accurately matching paragraph content
   - `references` list containing `"LINE-9925"`
1. **Exclusion of Rule Definitions**: Parser does NOT extract content where
   `is_rule_definition(tag)` is true (even if contains line references)
...
1. **Successful Execution**: Script runs without unhandled exceptions (exit code 0)
   - **Validation**: `python -m src.main --file t4002-6.html` (or CLI command) exits
     with code 0
1. **Artifact Generation**: Script produces structured output file (e.g.,
   `output/t4002-6/principles.yml`)
1. **Content Presence**: Output contains at least one object with
   `content_type: PRINCIPLE`
1. **Regression Test**: t4002-6.html produces 0 RULE objects (all references correctly
   filtered, validates Phase 1)
...
1. **New Section**: File contains new, clearly marked section for "Principle Content"
1. **Concrete Example**: Section includes:
...
1. **Structural**: `ExtractedContent` has `table_data: list[dict[str, str]] | None`
   field
1. **Quality**: `mypy` passes with no new errors
1. **Unit Test**: Can create
   `ExtractedContent(content_type=ContentType.TABLE, table_data=[...])`
   - **Validation**:
     `pytest tests/unit/extraction/ca/test_models.py::test_create_table_content`
...
1. **Quality**: BeautifulSoup only (no Ghostscript/other deps)
1. **Quality**: Passes `ruff` linting and formatting
1. **Validation**: `pytest tests/unit/extraction/ca/test_table_parser.py -v`
...
1. **Execution**: Command runs successfully (exit code 0)
1. **Execution**: Output file `output/t4002-10/tables.yml` created (valid YAML)
1. **Validation**: `grep -c "content_type: TABLE" output/t4002-10/tables.yml` returns
   `2`
1. **Validation**: First table's `table_data` field is non-empty list of dicts
1. **Quality**: Both tables have correct headers ("Property", "Class number")
1. **Regression**: No impact on existing `extract-rules` or `extract-principles`
   commands
...
1. **Validation**: Manual peer review - team member can understand table extraction
1. **Quality**: Markdown passes `mdformat` pre-commit hook

**Known Limitations (Baseline Quality - 80/20)**:

1. **Complex Structures**: No `colspan` or `rowspan` support (simple grid only)
1. **Nested Tables**: Extracted as flat, separate tables (loses hierarchy)
1. **Missing Semantic Tags**: Tables without `<thead>` or `<tbody>` skipped with warning
1. **No Caption/Title**: Does not extract `<caption>` tags or nearby headings
1. **Inconsistent Columns**: Assumes all rows have same cell count as headers
...
1. Build migration script for 63 production rules
1. Test with one file at a time
1. Gradually build up to full database
...
1. **t4002-6.html** - Mix of rules and references ✅ Use this second
1. **t4002-3.html** - Rich in PRINCIPLES, EXAMPLES ⏳ Use this third
1. **t4002-8/10/11.html** - Rich in TABLES (CCA rates) ⏳ Use for table extraction
1. **t4002-1.html** - Rich in GUIDANCE (chapter intro) ⏳ Use for guidance extraction
...
1. **Minimal**: Start with 2 types (RULE, PRINCIPLE), add one at a time
1. **Smart**: Learn from each file before processing next
1. **Documented**: Every step creates documentation
1. **Testable**: One file = one test case, clear acceptance criteria
1. **Incremental**: Each commit delivers value
