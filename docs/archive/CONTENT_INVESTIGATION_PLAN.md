...
**Goal**: Deliver value quickly (83 rules in database) while building understanding for
future multi-content-type extraction.

**Status**: Investigation Phase **Date**: 2025-10-24
...
- `t4002-3.html` → `t4002-3.html.CONTEXTUAL_ONLY` (principles/guidance, references
  rules)
...
1. Fix citation_id format: `{source_file}-LINE-{number}` (e.g., `t4002-4-LINE-9600`)
1. Extract 20 rules from t4002-4.html + t4002-6.html
1. Combine with 63 production rules from t4002-5.html
1. Release **data-v2025.11** with ~83 total rules
...
1. Document patterns for each content type
1. Create examples showing structure
1. Build foundation for future V2 extraction
...
**Outcome**: Full HTML content capture (rules + principles + tables + examples +
guidance)
...
**Special case**: Keep t4002-5.html using old format `LINE-{number}` for backwards
compatibility with production database.
...
**Commit**: "feat: combine production database (63 rules) with new extractions (20
rules)"
...
✅ Database has 83 rules (63 production + 20 new) ✅ No duplicate citation_id errors ✅ All
searches working ✅ Lineage traceable to source files ✅ GitHub release created
(data-v2025.11)

### Phase 2 (Foundation for Future)

✅ All 5 content types documented with examples ✅ HTML patterns identified ✅ Extraction
strategies defined ✅ Architecture document created ✅ Clear roadmap for V2 implementation
...
1. **Small commits**: One task = one commit
1. **Build incrementally**: Get database working first, then document
1. **Ask questions**: If anything unclear, pause and clarify before proceeding
