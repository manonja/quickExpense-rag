...
**Investigation Date**: 2025-10-24 **Resolution Date**: 2025-10-27 **Issue**: Database
build fails with duplicate `citation_id` errors **Duplicates Found**: LINE-9600,
LINE-9604 **Status**: ✅ RESOLVED (Phase 1 implementation)
...
**Classic Parser Logic Flaw**: Our parser looks for patterns like "Line 9XXX" in the
HTML text but doesn't distinguish between:

1. **Rule definitions**: Have anchor IDs (`id="tocch2ln9604"`), are headings, contain
   full content
1. **Rule references**: No anchor IDs, are links or mentions, point to definitions

Both get extracted as separate rules with same `citation_id = "LINE-9604"` → database
...
- **16 rules from t4002-4.html**: Likely ALL are canonical definitions (Chapter 2 –
  Income)
...
- **t4002-5.html**: 65 rules (classic parser) - Improved from 63 (found 2 missed
  combined headings) ✅
...
**Next Phase**: Phase 2 will implement Option 3 (multi-content-type extraction) to
capture references as PRINCIPLE content type.
